...
| 用語 | 定義 | 属する層 |
|------|------|---------|
| **tool_use** | LLMが出力するツール呼び出し宣言（JSON） | 第2層B（LLM起点） |
| **tool_result** | ホストがツール実行後に返す結果 | 第2層A（クライアント起点） |
...
| **RAG** | 外部知識をプロンプトに注入する技術 | 第2層A / 第4層 |
| **CLAUDE.md** | セッション開始時に自動注入されるMarkdown | 第2層A（クライアント起点） |
| **Skill** | 使用時のみロードされるプロンプトモジュール | 第2層A（クライアント起点） |
