...
| 技術/規格 | 第5層 | 第4層 | 第3層 | 第2層IN | 第2層OUT | 第1層 | 解説章 | 備考 |
|:--|:--:|:--:|:--:|:--:|:--:|:--:|:--|:--|
| **── LLM基盤技術 ──** |
...
| **── RAG ──** |
| 従来型RAG | - | 🔧 | **✅** | **✅** | - | - | 04_a | 3層が4層を使い2層INに注入 |
| ツール型RAG | - | **✅** | 🔧 | 🔧 | **✅** | 🔧 | 04_a | LLMが主体的に検索 |
| ベクトルDB | - | **✅** | 🔧 | - | - | - | 04_a | 4層の実装、3層が利用 |
| **── Claude Code固有 ──** |
| CLAUDE.md | - | - | 🔧 | **✅** | - | - | 04_c, 07_a | 3層が読み込み2層INに展開 |
| Skill（定義） | - | - | **✅** | 🔧 | - | - | 07_a | 3層でロード、2層INに注入 |
...
- **ツール利用判断**: ✅が2層OUT（LLM起点）、🔧が2層IN（Clientに返る）と1層（LLM機能依存）
- **ツール型RAG**: ✅が2層OUT（LLM主導）と4層（DB）、🔧が広範囲に影響
...
| tool descriptionが貧弱 | 2層IN→2層OUT | LLMがツールを使わない |
| MCPスキーマ不正 | 3層↔4層 | サーバー実装のJSON誤り |
| CLAUDE.mdが巨大 | 2層IN | コンテキスト汚染 |
| 従来RAGとTool型RAGの混同 | 2層IN vs 2層OUT | 主体が異なる |
| フレームワーク間通信 | 3層↔4層 | A2A未対応 |
| プロンプトインジェクション | 2層IN・3層 | 入力検証の不備 |
