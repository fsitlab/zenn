---
title: "7章：製品解説 (3) Dify：ノーコード/ローコードLLMアプリ開発プラットフォーム"
---
...
**概要**: ビジュアルエディタでノードをつなげてLLMアプリのロジックを構築。コード不要で複雑な処理フローを設計できる。
...
**概要**: ドキュメントをアップロードするだけで、自動的にチャンク分割・埋め込み生成・ベクトルDB保存を行い、検索可能な知識ベースを構築。
...
**概要**: LLMがタスクを分解し、必要なツールを自律的に呼び出して問題を解決するモード。ReActパターンまたはFunction Callingで実装。
...
**概要**: 構築したアプリをREST API / ストリーミングAPIとして即座に公開。外部システムからの呼び出しが可能。
