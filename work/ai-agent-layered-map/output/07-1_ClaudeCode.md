...
> **一言**: Anthropic公式のCLIコーディングエージェント。ターミナルから直接LLMと対話し、コードベース全体を理解・編集する。
...
**一言**: Claude CodeはAnthropic公式のCLIベースAIコーディングエージェント。ターミナルでの対話を通じて、コードの読み書き・リファクタリング・デバッグ・Git操作などを自然言語で実行できる。
...
**一言**: 親エージェントが特定タスクを委譲するために起動する子エージェント。
...
**一言**: 複数のエージェント（テイメイト）がタスクボードを共有し、非同期並列で協働する。
...
**一言**: Model Context Protocolにより、ツール定義を標準化・外部化。
...
**一言**: LLMの確率的判断を介さず、シェルスクリプトを確実に実行する決定論的制御。
...
**一言**: 使用時だけロードされるプロンプトのモジュール（Progressive Disclosure）。
